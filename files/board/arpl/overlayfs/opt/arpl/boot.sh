...
if [ "$DT" = "true" ] && [ $(lspci -nnk | grep -ie "\[0104\]" -ie "\[0107\]" | wc -l) -gt 0 ]; then
  ...
  echo "WARN: Device Tree Model selected - Raid/SCSI Controller not supported!"
  sleep 5
  return 1
else
  ...
  if [ -n "${WARNON}" ]; then
    echo "WARN: Your Controller has more than 8 Drives connected. Max Drives per Controller: 8"
    sleep 5
  fi
