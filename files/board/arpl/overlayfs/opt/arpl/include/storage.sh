...
function getmap() {
  ...
  for PCI in $(lspci -nnk | grep -ie "\[0106\]" | awk '{print $1}'); do
    ...
    DRIVES=$(ls -la /sys/block | grep -F "${PCI}" | grep -v "sr.$" | wc -l)
    unset HOSTPORTS
    declare -A HOSTPORTS
    ...
    [[ ${NUMPORTS} > 8 ]] && NUMPORTS=8
    [[ ${CONPORTS} > 8 ]] && CONPORTS=8
  ...
  if [[ ${MACHINE} = NATIVE ]]; then
    if [[ -n ${SATAREMAP} && ${SASCONTROLLER} = 0 ]]; then
      REMAP3="*"
    elif [[ -n ${SATAREMAP} && ${SASCONTROLLER} > 0 ]]; then
      REMAP2="*"
  ...
  if [[ ${SASCONTROLLER} > 0 ]]; then
    ...
    while true; do
      ...
      if [[ ${resp} = 1 ]]; then
        ...
        if [[ ${SASCONTROLLER} > 0 ]]; then
          dialog --backtitle "$(backtitle)" --title "Arc Disks" \
            --infobox "SAS Controller detected. Switch to SataPortMap (max Ports)!" 0 0
          writeConfigKey "arc.remap" "2" "${USER_CONFIG_FILE}"
        else
          dialog --backtitle "$(backtitle)" --title "Arc Disks" \
