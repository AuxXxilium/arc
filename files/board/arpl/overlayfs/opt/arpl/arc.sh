    ...
    while read -r M; do
      ...
      if [[ ${RESTRICT} -eq 1 ]]; then
        ...
        for F in "$(readModelArray "${M}" "dt")"; do
          if [[ ${DT} = true && ${SASCONTROLLER} > 0 ]]; then
            COMPATIBLE=0
            FLGNEX=1
            break
          fi
      ...
      [[ ${DT} = true ]] && DT="-DT" || DT=""
      [[ ${COMPATIBLE} = 1 ]] && echo -e "${MID} \"\Zb${DISKS}-Bay\Zn \t\Zb${CPU}\Zn \t\Zb${PLATFORM}${DT}\Zn \t\Zb${ARCAV}\Zn\" " >>"${TMP_PATH}/menu"
    done < <(find "${MODEL_CONFIG_PATH}" -maxdepth 1 -name \*.yml | sort)
    [[ ${FLGBETA} = 0 ]] && echo "b \"\Z1Show beta Models\Zn\"" >>"${TMP_PATH}/menu"
    [[ ${FLGNEX} = 1 ]] && echo "f \"\Z1Show incompatible Models \Zn\"" >>"${TMP_PATH}/menu"
...
function arcnetdisk() {
  ...
  if [[ ${SATACONTROLLER} > 0 && ${DT} != true ]]; then
    ...
    while true; do
      ...
      case "$(<${TMP_PATH}/resp)" in
        ...
        4)
          ...
          for F in $(ls -A); do
            USER_FILE="${F}"
            [[ "${F##*.}" = "zip" && $(unzip -l ${TMP_PATH}/${USER_FILE} | grep -c "\.img$") = 1 ]] && IFTOOL="zip"
...
function sysinfo() {
  ...
  if [[ ${SASCONTROLLER} > 0 ]]; then
    for PCI in $(lspci -nnk | grep -ie "\[0104\]" -ie "\[0107\]" | awk '{print$1}'); do
      # Get Name of Controller
      NAME="$(lspci -s "${PCI}" | sed "s/\ .*://")"
      # Get Amount of Drives connected
      SASDRIVES=$(ls -la /sys/block | grep -F "${PCI}" | grep -v "sr.$" | wc -l)
...
while true; do
  ...
  if [[ -n ${CONFDONE} ]]; then
    ...
    if [[ -n ${ARCOPTS} ]]; then
      ...
      if [[ ${DT} != true && ${SATACONTROLLER} > 0 ]]; then
  ...
  case $(<${TMP_PATH}/resp) in
    ...
    k)
      [[ ${KERNELLOAD} = power ]] && KERNELLOAD='kexec' || KERNELLOAD='power'
