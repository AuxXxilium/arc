...
function arcdiskconf() {
    ...
    writeConfigKey "maxdisks" "$maxdisks" "${USER_CONFIG_FILE}"
    dialog --backtitle "`backtitle`" --title "ARC Config" \
      --infobox "ARC Disk configuration successfull! MaxDisks=${maxdisks} | SataPortMap=${sataportmap} | DiskIdxMap=${diskidxmap}" 0 0  
    sleep 5
    arcnet
}
