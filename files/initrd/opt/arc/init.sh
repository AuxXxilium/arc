...
initConfigKey "arc.msifix" "false" "${USER_CONFIG_FILE}"
...
initConfigKey "iommupt" "false" "${USER_CONFIG_FILE}"
...
initConfigKey "modulesunload" "false" "${USER_CONFIG_FILE}"
...
# lighttpd can leave its pidfile behind when it dies, so check the process, not the file
if [ ! -f /var/run/lighttpd.pid ] || ! kill -0 "$(cat /var/run/lighttpd.pid 2>/dev/null)" 2>/dev/null; then
  rm -f /var/run/lighttpd.pid
  /etc/init.d/S90lighttpd restart >/dev/null 2>&1 && RESTARTED=1
fi
[ "${RESTARTED}" = "1" ] && sleep 5
IPCON=""
checkNIC
