  ...
  if [ "${CUSTOM}" == "false" ]; then
    while true; do
      ...
      while read -r M A; do
        ...
        [ "${DT}" == "true" ] && DTS="x" || DTS=""
        IGPUS=""
        [[ "${A}" == "apollolake" || "${A}" == "geminilake" ]] && IGPUS="up to 10th" || "${A}" == "epyc7002" ]] && IGPUS="up to 14th" 
        [ "${DT}" == "true" ] && HBAS="" || HBAS="x"
        [ "${M}" == "SA6400" ] && HBAS="x"
        ...
        if [ -n "${ARC_KEY}" ]; then
          ...
          [ ${COMPATIBLE} -eq 1 ] && echo -e "${M} \"\t$(printf "\Zb%-15s\Zn \Zb%-5s\Zn \Zb%-5s\Zn \Zb%-12s\Zn \Zb%-10s\Zn \Zb%-12s\Zn \Zb%-10s\Zn \Zb%-10s\Zn" "${A}" "${DTS}" "${IGPUS}" "${HBAS}" "${M_2_CACHE}" "${M_2_STORAGE}" "${USBS}" "${BETA}")\" ">>"${TMP_PATH}/menu"
        fi
      ...
      if [ -n "${ARC_KEY}" ]; then
        dialog --backtitle "$(backtitle)" --title "Arc DSM Model" --colors \
          ...
          --menu "Supported Models for your Hardware (x = supported / + = need Addons)\n$(printf "\Zb%-16s\Zn \Zb%-15s\Zn \Zb%-5s\Zn \Zb%-5s\Zn \Zb%-5s\Zn \Zb%-12s\Zn \Zb%-10s\Zn \Zb%-12s\Zn \Zb%-10s\Zn \Zb%-10s\Zn" "Model" "Platform" "DT" "Arc" "i915" "HBA" "M.2 Cache" "M.2 Volume" "USB Mount" "Source")" 0 110 0 \
          --file "${TMP_PATH}/menu" 2>"${TMP_PATH}/resp"
      else
        dialog --backtitle "$(backtitle)" --title "DSM Model" --colors \
          --cancel-label "Show all" --help-button --help-label "Exit" \
          --extra-button --extra-label "Info" \
          --menu "Supported Models for your Hardware (x = supported / + = need Addons) | Syno Models can have faulty Values.\n$(printf "\Zb%-16s\Zn \Zb%-15s\Zn \Zb%-5s\Zn \Zb%-5s\Zn \Zb%-12s\Zn \Zb%-10s\Zn \Zb%-12s\Zn \Zb%-10s\Zn \Zb%-10s\Zn" "Model" "Platform" "DT" "i915" "HBA" "M.2 Cache" "M.2 Volume" "USB Mount" "Source")" 0 110 0 \
          --file "${TMP_PATH}/menu" 2>"${TMP_PATH}/resp"
      fi
      RET=$?
      case ${RET} in
