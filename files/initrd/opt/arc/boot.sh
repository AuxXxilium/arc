...
# The sortnetif addon sorts interfaces by bus-id and can pin specific NICs to
# the front of that order. Pass the configured MAC list on the cmdline rather
# than as an addon parameter: addons.sh is generated by ramdisk-patch.sh at
# build time, so a parameter would freeze the value into the ramdisk and only
# take effect after a full rebuild, while the cmdline is written here on every
# boot and picks the config up straight away. This mirrors how netfix reads its
# R<bus>=<mac> entries below.
SORTNETIF="$(readConfigKey "addons.sortnetif" "${USER_CONFIG_FILE}")"
[ -n "${SORTNETIF}" ] && CMDLINE['sortnetif']="$(echo "${SORTNETIF}" | sed 's/://g; s/ //g' | tr '[:upper:]' '[:lower:]')" || true
