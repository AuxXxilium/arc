...
function getBuildrootx() {
  ...
  TAG="$(curl -skL https://api.github.com/repos/AuxXxilium/arc-buildroot-x/releases/latest | grep -oP '"tag_name": "\K(.*)(?=")')"
  export BRXTAG="${TAG}"
...
function getBuildroots() {
  ...
  TAG="$(curl -skL https://api.github.com/repos/AuxXxilium/arc-buildroot-s/releases/latest | grep -oP '"tag_name": "\K(.*)(?=")')"
  export BRSTAG="${TAG}"
  [ ! -d "${DEST_PATH}" ] && mkdir -p "${DEST_PATH}"
  echo "Getting Kernel"
  rm -f "${DEST_PATH}/bzImage-arc"
