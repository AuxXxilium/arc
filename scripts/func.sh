...
function getBuildrootx() {
  ...
  TAG="$(curl -skLH "Authorization: Bearer ${{ secrets.BUILD_TOKEN }}" https://api.github.com/repos/AuxXxilium/arc-buildroot-x/releases/latest | grep -oP '"tag_name": "\K(.*)(?=")')"
  export BRXTAG="${TAG}"
  [ ! -d "${DEST_PATH}" ] && mkdir -p "${DEST_PATH}"
  echo "Getting Kernel"
  rm -f "${DEST_PATH}/bzImage-arc"
  if curl -skLH "Authorization: Bearer ${{ secrets.BUILD_TOKEN }}" "https://github.com/AuxXxilium/arc-buildroot-x/releases/download/${TAG}/bzImage" -o "${DEST_PATH}/bzImage-arc"; then
    echo "Kernel: ${TAG}"
  else
  ...
  if curl -skLH "Authorization: Bearer ${{ secrets.BUILD_TOKEN }}" "https://github.com/AuxXxilium/arc-buildroot-x/releases/download/${TAG}/rootfs.cpio.xz" -o "${DEST_PATH}/initrd-arc"; then
    echo "Ramdisk: ${TAG}"
  else
...
function getBuildroots() {
  ...
  TAG="$(curl -skLH "Authorization: Bearer ${{ secrets.BUILD_TOKEN }}" https://api.github.com/repos/AuxXxilium/arc-buildroot-s/releases/latest | grep -oP '"tag_name": "\K(.*)(?=")')"
  export BRSTAG="${TAG}"
  [ ! -d "${DEST_PATH}" ] && mkdir -p "${DEST_PATH}"
  echo "Getting Kernel"
  rm -f "${DEST_PATH}/bzImage-arc"
  if curl -skLH "Authorization: Bearer ${{ secrets.BUILD_TOKEN }}" "https://github.com/AuxXxilium/arc-buildroot-s/releases/download/${TAG}/bzImage" -o "${DEST_PATH}/bzImage-arc"; then
    echo "Kernel: ${TAG}"
  else
    echo "Failed to get Kernel"
    exit 1
  ...
  echo "Getting Ramdisk"
  rm -f "${DEST_PATH}/initrd-arc"
  if curl -skLH "Authorization: Bearer ${{ secrets.BUILD_TOKEN }}" "https://github.com/AuxXxilium/arc-buildroot-s/releases/download/${TAG}/rootfs.cpio.xz" -o "${DEST_PATH}/initrd-arc"; then
    echo "Ramdisk: ${TAG}"
  else
    echo "Failed to get Ramdisk"
    exit 1
